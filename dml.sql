...
FROM
    Suppliers AS T1
JOIN
    Products AS T2 ON T1.SupplierID = T2.SupplierID
WHERE
    T2.CategoryID = 8; 
...
FROM
    Orders AS T1
JOIN
    Shippers AS T2 ON T1.ShipperID = T2.ShipperID
GROUP BY
    T2.ShipperName
ORDER BY
    TotalOrders DESC;
...
WHERE
    T1.OrderDate LIKE '%/12/1997'
GROUP BY
    T1.CustomerID, T2.CustomerName
HAVING
    COUNT(T1.OrderID) > 3;
...
FROM
    Products AS T1
JOIN
    OrderDetails AS T2 ON T1.ProductID = T2.ProductID
GROUP BY
    T1.ProductID, T1.ProductName
HAVING
    SUM(T2.Quantity) < 50;